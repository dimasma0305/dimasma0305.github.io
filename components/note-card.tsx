...
export default function NoteCard({ note, className }: NoteCardProps) {
  return (
    <Card
      className={cn(
        "note-card group relative overflow-hidden transition-[transform,box-shadow] duration-[var(--dur-base)] ease-[var(--ease-out)] hover:-translate-y-1 hover:shadow-[var(--elevation-2)]",
        className,
      )}
    >
      <CardContent className="p-4 flex flex-col">
        ...
        {/* Title */}
        <h2 className="note-card-title text-xl font-bold mb-2 line-clamp-2 [overflow-wrap:anywhere] group-hover:text-primary transition-colors">
          {note.title}
        </h2>

        {/* Excerpt */}
        {note.excerpt && (
          <p className="text-sm text-muted-foreground mb-4 line-clamp-3 [overflow-wrap:anywhere]">
            {note.excerpt}
          </p>
        )}

        {/* Footer */}
        <div className="note-card-footer flex items-center justify-between pt-4 border-t border-border/50 mt-4">
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <div className="flex items-center gap-1">
              <Calendar className="h-3 w-3" />
              <time dateTime={note.created_time}>
                {format(new Date(note.created_time), "MMM d, yyyy")}
              </time>
            </div>
            {note.reading_time && (
              <div className="flex items-center gap-1">
