...
// <picture> with an AVIF twin ahead of the WebP the markup already carries.
const picture = (img, avif, attrs = "") =>
  avif
    ? `<picture><source type="image/avif" srcset="${e(avif)}"${attrs} />${img}</picture>`
    : img;
...
const projectStory = (item, story, index, site) =>
  `<details class="tour-expandable tour-entry tour-project-entry tour-project-case" id="project-${index}" data-portfolio-project><summary><img src="${e(site + optimizedStoryImage(story.image))}" width="${story.width}" height="${story.height}" loading="lazy" decoding="async" alt=""/><span><small>${e(story.label)}</small><strong>${e(item.title)}</strong><span class="tour-case-takeaway">${e(story.takeaway)}</span></span><span aria-hidden="true">+</span></summary><div class="tour-expansion"><dl class="tour-case-facts"><dt>The problem</dt><dd>${e(story.problem)}</dd><dt>${item.team ? "The contribution" : "My work"}</dt><dd>${e(story.contribution)}</dd><dt>The result</dt><dd>${e(story.outcome)}</dd></dl><p class="tour-item-date">${e(item.date)}${item.team ? ` · ${e(item.team)}` : ""}</p><figure class="tour-case-image"><a href="${e(site + optimizedStoryImage(story.image))}" aria-label="View full preview of ${e(item.title)}"><img src="${e(site + optimizedStoryImage(story.image))}" width="${story.width}" height="${story.height}" loading="lazy" decoding="async" alt="${e(story.caption)}"/></a><figcaption>${e(story.caption)}</figcaption></figure>${tags(item.tags)}<a class="tour-link" href="${e(item.github)}">Source & documentation on GitHub ↗</a></div></details>`;
const date = (value) =>
...
const photo = (item, index) =>
  `<figure data-portfolio-photo id="tour-photo-${index}"${index ? " hidden" : ""}><a href="${e(item.original)}" aria-label="View full photo: ${e(item.alt)}">${picture(`<img src="${e(item.src)}"${roomPhotoSrcset(item) ? ` srcset="${e(roomPhotoSrcset(item))}" sizes="(max-width: 900px) 100vw, 740px"` : ""} width="1200" height="800" loading="lazy" decoding="async" alt="${e(item.alt)}"/>`, avifSrcset(roomPhotoSrcset(item)), ' sizes="(max-width: 900px) 100vw, 740px"')}</a><figcaption>${item.chip ? `<strong>${e(item.chip)}</strong> ` : ""}${e(item.caption || item.alt)}</figcaption></figure>`;
...
export function tourContent({ email, site, portfolio: p }) {
  ...
  return {
    ...
    about: `<p class="tour-section-intro">From Bali to Vietnam and China. A few moments with the people behind the work.</p>
      <div class="tour-photo-feature tour-album"><div class="tour-album-stage">${p.photos.map(photo).join("")}</div><div class="tour-album-controls" role="group" aria-label="Choose a photo from the album">${p.photos.map((item, i) => `<button type="button" data-tour-photo="${i}" aria-pressed="${i === 0}" aria-controls="tour-photo-${i}" aria-label="Photo ${i + 1}: ${e(item.chip || item.alt)}">${picture(`<img src="${e(item.thumb || item.src)}" width="160" height="100" loading="lazy" decoding="async" alt=""/>`, item.thumb && roomAvif(item.thumb) !== item.thumb ? roomAvif(item.thumb) : "")}<span>${e(item.chip || `Moment ${i + 1}`)}</span></button>`).join("")}</div><p class="tour-album-count" id="tour-album-status" role="status">1 / ${p.photos.length} · ${e(p.photos[0].chip || p.photos[0].alt)}</p></div>
