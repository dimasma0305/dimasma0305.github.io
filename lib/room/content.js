...
const photo = (item, index) =>
  `<figure data-portfolio-photo id="tour-photo-${index}"${index ? " hidden" : ""}><a href="${e(item.original)}" aria-label="View full photo: ${e(item.alt)}"><img src="${e(item.src)}"${roomPhotoSrcset(item) ? ` srcset="${e(roomPhotoSrcset(item))}" sizes="(max-width: 900px) 100vw, 740px"` : ""} width="1200" height="800" loading="lazy" decoding="async" alt="${e(item.alt)}"/></a><figcaption>${item.chip ? `<strong>${e(item.chip)}</strong> ` : ""}${e(item.caption || item.alt)}</figcaption></figure>`;
...
export function tourContent({ email, site, portfolio: p }) {
  ...
  return {
    ...
    about: `<p class="tour-section-intro">From Bali to Vietnam and China. A few moments with the people behind the work.</p>
      <div class="tour-photo-feature tour-album"><div class="tour-album-stage">${p.photos.map(photo).join("")}</div><div class="tour-album-controls" role="group" aria-label="Choose a photo from the album">${p.photos.map((item, i) => `<button type="button" data-tour-photo="${i}" aria-pressed="${i === 0}" aria-controls="tour-photo-${i}" aria-label="Photo ${i + 1}: ${e(item.chip || item.alt)}"><img src="${e(item.thumb || item.src)}" width="160" height="100" loading="lazy" decoding="async" alt=""/><span>${e(item.chip || `Moment ${i + 1}`)}</span></button>`).join("")}</div><p class="tour-album-count" id="tour-album-status" role="status">1 / ${p.photos.length} · ${e(p.photos[0].chip || p.photos[0].alt)}</p></div>
