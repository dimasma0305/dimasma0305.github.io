    ...
    <main id="main-content" tabindex="-1">
      <div class="tour-run">
        <div class="tour-stage">
          <div id="tour-scene" class="tour-scene" data-state="loading"><picture><source type="image/avif" srcset="${escapeHTML(avifSrcset(roomStillSrcset("assets/room-stills/welcome.jpg", (path) => roomAsset(assetBase, path))))}" sizes="min(100vw, 100vh)" /><img class="tour-still-image" src="${escapeHTML(roomAsset(assetBase, "assets/room-stills/welcome.jpg"))}" srcset="${escapeHTML(roomStillSrcset("assets/room-stills/welcome.jpg", (path) => roomAsset(assetBase, path)))}" sizes="min(100vw, 100vh)" width="2048" height="2048" fetchpriority="high" alt="" /></picture>
        ...
        <div class="tour-chapters">
          <section id="welcome" class="tour-chapter tour-welcome" data-side="right" aria-labelledby="tour-title" tabindex="-1">${alias("welcome")}<div class="tour-copy tour-intro">
            ...
            <div class="tour-signature"><picture><source type="image/avif" srcset="${escapeHTML(roomAvif(roomAsset(assetBase, "assets/dimas.jpg")))}" /><img src="${escapeHTML(roomAsset(assetBase, "assets/dimas.jpg"))}" width="46" height="46" alt="Dimas’ illustrated avatar"/></picture><div><strong>Research. Build. Share.</strong><span>A personal space, from Bali.</span></div></div>
          </div></section>
...
export function initCornerTour({
  ...
  // The clone only ever shows a still that is already on screen. It receives
  // that URL when a fade starts, so it must not select a candidate of its own.
  for (const name of ["fetchpriority", "src", "srcset", "sizes"]) {
    outgoingImage.removeAttribute(name);
  }
  outgoingImage.className = "tour-still-outgoing";
  outgoingImage.hidden = true;
  // Outside the <picture>, which holds exactly one image.
  (stillImage.closest("picture") || stillImage).after(outgoingImage);
  // Script-driven swaps choose the format themselves; WebP until known.
  let avifReady = false;
  ...
  // Largest width already fetched per still, seeded by the browser's own
  // `srcset` choice for the first view.
  const stillWidths = new Map();
  if (stillImage.currentSrc) {
    const chosen = /-(\d+)\.(?:avif|webp)$/.exec(stillImage.currentSrc);
    stillWidths.set(
      roomStills.welcome.src,
      chosen ? Number(chosen[1]) : roomStillFullWidth,
    );
  }
  let frames = [],
    centers = [],
  ...
  // Smallest render with at least one source pixel per device pixel. A still
  // only moves up to a larger file, so one view never fetches two candidates.
  function stillUrl(index) {
    const stop = stops[index];
    const item = roomStills[stop?.id];
    if (!item) {
      return "";
    }
    const { size } = tourStillBox(
      stop,
      sceneWidth,
      sceneHeight,
      !mobile.matches,
    );
    const needed = size * (devicePixelRatio || 1);
    const width = Math.max(
      roomStillWidths.find((candidate) => candidate >= needed) ||
        roomStillFullWidth,
      stillWidths.get(item.src) || 0,
    );
    stillWidths.set(item.src, width);
    const webp = roomAsset(
      assetBase,
      width >= roomStillFullWidth
        ? item.src
        : roomStillVariant(item.src, width),
    );
    return avifReady ? roomAvif(webp) : webp;
  }
  function preloadStill(index) {
    const url = stillUrl(index);
    if (!url) {
  ...
  function showStill() {
    ...
    // A larger window can need a larger render of the still already shown.
    const request = `${key} ${stillUrl(active)}`;
    if (requestedStill === request) {
      return;
    }
    requestedStill = request;
    preloadStill(active).then((img) => {
      if (disposed || !still || requestedStill !== request || !img) {
        return;
      }
      ...
      // From here the candidate is chosen above; one task, one selection.
      stillImage.removeAttribute("srcset");
      stillImage.removeAttribute("sizes");
      for (const source of stillImage
        .closest("picture")
      ...
      // Warm the next stop only once this one is on screen, so the prefetch
      // never competes with the still a visitor is waiting for.
