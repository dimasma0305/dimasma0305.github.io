...
import { supportsAvif } from "../image-format.js";
...
export function createCornerScene({
  ...
  // Same call shape as TextureLoader; loads the AVIF twin where it decodes.
  const textureLoader = {
  ...
  function photo(path, w, h, pos, parent) {
    ...
    textureLoader.load(
      new URL(optimizedRoomPath(path), assetBase).href,
      (t) => {
        if (disposed) {
          t.dispose();
          return;
  ...
  // Every light is anchored to something visible in the room. The broad
  // hemisphere only keeps the unlit faces legible; it must not become the
  // apparent source of the scene.
  const ambient = new THREE.HemisphereLight(0xe7eff4, 0x263229, 1.25);
  const sunlight = new THREE.DirectionalLight(0xffe4ad, 3.1);
  // This key follows the same path as the light entering through the window.
  // Keeping the source just inside the modeled wall lets the window frame cast
  // long shadows without the diorama's solid backing wall occluding the room.
  sunlight.position.set(-3.05, 4.8, -1.08);
  sunlight.target.position.set(0.35, 0.72, 0.35);
  ...
  // A window-sized emitter softens the directional key and makes its origin
  // readable in reflections and on surfaces that do not face the moon key.
  const fill = new THREE.RectAreaLight(0xc4d8e4, 2, 1.58, 1.92);
  fill.position.set(-3.12, 2.76, -1.08);
  fill.lookAt(0.2, 1.72, 0.05);

  // The bulb is at the center of the modeled lamp shade. A spotlight respects
  // that shade, while a small area emitter over the lit desk approximates the
  // warm light reflected back into the room.
  ...
  lampLight.position.set(-2.02, 2.492, -1.17);
  lampLight.target.position.set(-1.68, 1.59, -1.06);
  lampLight.castShadow = true;
  lampLight.shadow.mapSize.set(512, 512);
  lampLight.shadow.camera.near = 0.08;
  lampLight.shadow.camera.far = 4.4;
  lampLight.shadow.focus = 0.82;
  lampLight.shadow.bias = -0.00015;
  lampLight.shadow.normalBias = 0.018;
  lampLight.shadow.radius = 3;
  const lampBounce = new THREE.RectAreaLight(0xffc987, 0, 1.46, 0.76);
  lampBounce.position.set(-1.67, 1.625, -1.04);
  lampBounce.lookAt(-0.15, 2.42, 0.72);

  // The monitor is itself a rectangular emitter, so its spill now matches the
  // size and direction of the visible screen instead of glowing in all axes.
  const screenLight = new THREE.RectAreaLight(0x9fcfc2, 0.35, 1.5, 0.82);
  screenLight.position.set(-0.8, 2.495, -0.985);
  screenLight.lookAt(-0.72, 2.02, 1.15);

  // Three short-range lights represent the shared output of the eleven string
  // bulbs. Their inverse-square falloff creates separate pools on the back wall
  // without paying for eleven lights or washing the lower half of the room.
  const stringLights = [
    [-2.43, 3.935, -2.44],
    [-0.07, 3.79, -2.44],
    [2.29, 3.935, -2.44],
  ].map(([x, y, z]) => {
    const light = new THREE.PointLight(0xffbd72, 0, 1.7, 2);
  ...
  host.dataset.lightSources = "window,lamp,monitor,string";
  ...
  if (nightWindow) {
    ...
    if (craft) {
      textureLoader.load(
      new URL(optimizedRoomPath("./assets/bali-night-window.png"), assetBase).href,
        (texture) => {
          if (disposed) {
            texture.dispose();
            return;
  ...
  bulb.rotation.z = 0.2;
  // The light starts inside this mesh; allowing the bulb to cast would shadow
  // its own beam before it can leave the shade.
  ...
  function polaroid(path, x, y, rotation, caption) {
    ...
    // A pinned print never fills more than a few hundred pixels on screen.
    photo(roomPhotoMedium(path), 0.66, 0.49, [0, 0.054, 0.013], frame);
      ...
      .then((avif) =>
        Promise.all(
          moments.map(
            (item) =>
              new Promise((resolve) => {
                // Each cell is about 236px wide; the album thumbnail covers it.
                const file = item.thumb || item.src;
                imageLoader.load(
                  new URL(avif ? roomAvif(file) : file, assetBase).href,
                  resolve,
                  undefined,
                  () => resolve(null),
                );
  ...
  const lightingFields = [
    ...
    [fill, "color"],
    [lampLight, "intensity"],
    [lampBounce, "intensity"],
    [screenLight, "intensity"],
    [screenLight, "color"],
    ...stringLights.map((light) => [light, "intensity"]),
  ...
  function setTourView(view, immediate = false) {
    ...
    if (portfolioDetails && view.atmosphere) {
      ...
      screenLight.intensity = 1.25 + work * 0.8;
      lampLight.intensity = 34 + writing * 7;
      lampBounce.intensity = 1.8 + writing * 0.9;
  ...
  function setEvening(value) {
    ...
    ambient.intensity = value ? 0.62 : 1.25;
    ambient.color.set(value ? 0xb7c9ed : 0xe7eff4);
    sunlight.intensity = value ? 1.0 : 3.1;
    sunlight.color.set(value ? 0xccb0cd : 0xffe4ad);
    fill.intensity = value ? 1.8 : 2;
    fill.color.set(value ? 0xa8c5d7 : 0xc4d8e4);
    lampLight.intensity = value ? 30 : 0;
    lampBounce.intensity = value ? 1.55 : 0;
    screenLight.intensity = value ? 1.25 : 0.35;
    stringLights.forEach((light) => {
      light.intensity = value ? 0.9 : 0;
    });
    ...
    if (theme === "midnight" && value) {
      ...
      fill.color.set(0x9ebdcc);
      fill.intensity = 2.1;
      lampLight.intensity = 34;
      lampBounce.intensity = 1.8;
      screenLight.intensity = 1.25;
      stringLights.forEach((light) => {
    ...
    if (theme === "slate") {
      ...
      lampBounce.intensity = value ? 1.4 : 0;
      stringLights.forEach((light) => {
        light.intensity = value ? 0.7 : 0;
      });
