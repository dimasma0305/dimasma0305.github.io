...
// Fallback for legacy sections without the shared responsive header.
export const HEADER_OFFSET = 80;

export function getHeaderOffset(): number {
  if (typeof document === "undefined") return HEADER_OFFSET;
  const header = document.querySelector<HTMLElement>(".site-global-header");
  return header
    ? Math.ceil(header.getBoundingClientRect().height) + 24
    : HEADER_OFFSET;
}
...
export function smoothScrollToElement(
  ...
  return new Promise((resolve) => {
    const run = (element: HTMLElement) => {
      ...
      window.scrollTo({
        top: want,
        behavior: far || reducedMotion ? "instant" : behavior,
      });
