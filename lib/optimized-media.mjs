...
// Every generated room WebP (stills and their sizes, photos and their sizes,
// the avatar, the window) also has an AVIF twin at about half the bytes.
// Lossless screenshots are excluded: AVIF is larger than WebP for those.
const roomWebp =
  /(assets\/(?:room-stills\/[a-z]+(?:-\d+)?|(?:team-(?:bali|china)|portfolio\/photo-\d+)(?:-\d+|\.thumb)?|dimas|bali-night-window))\.webp$/;
export function roomAvif(source) {
  return source.replace(roomWebp, "$1.avif");
}
/** The same candidate list with every WebP swapped for its AVIF twin. */
export function avifSrcset(srcset) {
  if (!srcset) return "";
  const avif = srcset
    .split(", ")
    .map((candidate) => {
      const [url, descriptor] = candidate.split(" ");
      return [roomAvif(url), descriptor].filter(Boolean).join(" ");
    })
    .join(", ");
  return avif === srcset ? "" : avif;
}
