import { pageMetadata, siteUrls, serializeJsonLd } from "@/lib/site-seo";
...
function readBlogIndex(): { tags: IndexTag[]; posts: IndexPost[] } {
  try {
    ...
    const posts: IndexPost[] =
      (Array.isArray(blogIndex.posts?.published) &&
        blogIndex.posts.published) ||
  ...
  return (
    <div className="container px-4 py-12 mx-auto max-w-7xl">
      ...
      <BreadcrumbStructuredData
        items={[
          { name: "Tags", path: "/tags/" },
          { name: `#${name}`, path: `/tags/${tagToSlug(name)}/` },
        ]}
      />
      ...
      {tagPosts.length > 0 ? (
        ...
        <div className="text-center py-12">
          <p className="text-muted-foreground">No posts tagged {name} yet.</p>
        </div>
      )}
    </div>
  );
